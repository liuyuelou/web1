$(function() {
    ...
    $(".Mst_main_top4_left>a").click(function() {
        ...
        if ($(this).index() == 3) {
            let app = [];
            let udb = [];
            for (let i = 0; i < $(".mst_mk").length; i++) { //首先循环出来这个对象
                udb.push($(".mst_mk")[i]); //放到一个数组里面
            }
            while (udb.length > 0) {
                let index = Math.floor(Math.random() * udb.length);
                udb.splice(index, 1);
                app.push(udb[index]);
            }
            for (let i = 0; i < app.length; i++) {
                $(app[i]).appendTo($(".mainbox"));
            }
    ...
    $(window).scroll(function() { //此功能实现懒加载
