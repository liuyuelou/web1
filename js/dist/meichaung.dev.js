...
$(function () {
  ...
  console.log(bnarr); // img/上箭头 666.png
  ...
  $(".Mst_main_top4_left>a").click(function () {
    ...
    if ($(this).index() == 3) {
      ...
      for (var _i = 0; _i < $(".mst_mk").length; _i++) {
        //首先循环出来这个对象
        _udb.push($(".mst_mk")[_i]); //放到一个数组里面

      }

      while (_udb.length > 0) {
        var index = Math.floor(Math.random() * _udb.length);
      ...
      for (var _i2 = 0; _i2 < app.length; _i2++) {
        $(app[_i2]).appendTo($(".mainbox"));
      }
    }
      ...
      if (udb == true) {
        ...
        for (var k = 0; k < bkbr.length; k++) {
          for (var b = 0; b < bkbr.length - 1 - k; b++) {
            if (bkbr[b] > bkbr[b + 1]) {
              var _ref3 = [bkbr[b + 1], bkbr[b]];
              bkbr[b] = _ref3[0];
              bkbr[b + 1] = _ref3[1];
              var _ref4 = [objbbr[b + 1], objbbr[b]];
              objbbr[b] = _ref4[0];
              objbbr[b + 1] = _ref4[1];
            }
          }
  ...
  var lang = 17.2; //创建一个变量

  $("#right_btnhh").mousedown(function (e) {
    lang = 17.2; //17.2是用户ul的高度除以右边竖线的高度求出的
  ...
  $(".eng_box_right").mousemove(function (e) {
    //移动
    if (nb) {
      if (e.pageY < 280 && e.pageY > 121) {
        $("#right_btnhh").css("top", Math.floor(e.pageY - 118));
        console.log(Math.floor(e.pageY - 118));
        $(".eng_box_left>ul").css("top", Math.floor(-Math.floor(e.pageY - 123) * lang) + "px");
        console.log(e.pageY);
      }
    }
  });
  ...
  $(".nbkb").hover(function () {
    $(this).find(".nbdiv").show();
    $(this).find(".boxbotm_img img").hide();
  }, function () {
    $(this).find(".nbdiv").hide();
    $(this).find(".boxbotm_img img").show();
  });
  $(window).scroll(function () {
    //此功能实现懒加载
    if ($(this).scrollTop() + $(window).height() > $(".main3").height() && $(".main3").height() < 15802) {
      $(".main3").height($(this).scrollTop() + $(window).height());
    }
  });
});
$(function () {
